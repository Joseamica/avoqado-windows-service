...
- `AvoqadoProcessedCommands` is the command-idempotency store (deduped by `CommandKey` = `messageId`/`commandId`/`idempotencyKey`). As of commit `9c9da4a` the claim is **atomic with the side effect**: `claimCommand` (`src/core/commandDedup.ts`) runs as the first statement inside each non-idempotent adapter method's own transaction, so the claim row and the POS write commit/roll back together (no double-apply window). Check-then-insert: a redelivered key throws `CommandAlreadyProcessedError` before any INSERT → Commander acks+skips (not DLQ); the PK still guards the rare concurrent case (loser → DLQ, effect applied once). Fail-open if the table is absent. Covers the six non-idempotent commands (`Order.CREATE`, `OrderItem.CREATE`, `Shift.OPEN`, `FastPayment.CREATE`, `OrderItem.CANCEL`, `Order.SPLIT`). Conditional on the backend sending a stable `commandId` (dormant/fail-open today without one).
