...
- **`AvoqadoProcessedCommands`** - Command-idempotency store (deduped by `CommandKey` = `messageId` / `commandId` / `idempotencyKey`). As of commit `9c9da4a` the claim is **atomic with the side effect**: `claimCommand` (`src/core/commandDedup.ts`) is called as the FIRST statement inside each non-idempotent adapter method's OWN transaction, so the claim row and the POS write commit or roll back together (no double-apply window). It is **check-then-insert**: a redelivered command whose key already exists throws `CommandAlreadyProcessedError` before any INSERT → the Commander acks and skips (not DLQ); the `CommandKey` PK still guards the rare concurrent-in-flight case (loser PK-violates → tx aborts → DLQ, effect still applied exactly once). **Fail-open** if the table is absent (old install). Covers the six non-idempotent commands (`Order.CREATE`, `OrderItem.CREATE`, `Shift.OPEN`, `FastPayment.CREATE`, `OrderItem.CANCEL`, `Order.SPLIT`). **Conditional on the backend sending a stable `commandId`** — without one, `commandKey` is undefined → claim skipped → no dedup (dormant today). Pruned by `sp_CleanupOldTrackingRecords` (`ProcessedAt < cutoff`).
