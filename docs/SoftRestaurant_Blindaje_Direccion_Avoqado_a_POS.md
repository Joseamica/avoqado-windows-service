...
**Estado global de la ejecución TPV→SoftRestaurant: NO está a prueba de errores.** La superficie de comandos cableados es de **8** (`Order.CREATE`, `OrderItem.CREATE`, `Payment.APPLY`, `Shift.OPEN`, `Shift.CLOSE`, `FastPayment.CREATE`, y desde **Fase 5c** `OrderItem.CANCEL` y `Order.SPLIT`), de los cuales **uno está completamente roto** (`Shift.CLOSE`) y los demás funcionan **con caveats serios** (los dos de 5c sin idempotencia de operación; split validado solo en v11). Buena parte de la mitad transaccional del puente sigue sin existir —cancelar orden completa/void, descuentos, modificadores, juntar/transferir cuentas, cambiar mesa, imprimir— (cancelar línea y dividir cuenta **ya existen** desde 5c).
...
> **Fase 5c — cablear cancel + split (2026-06-15, ◑ parcial):** se cablearon dos operaciones cuyo SQL ya existía (sin objetos de DB nuevos). **(1)** `OrderItem.CANCEL` ahora está **cableado** en `commander.ts` (case con llaves → `adapter.cancelOrderItem(orderFolio, movementId, reason ?? 'Cancelado vía Avoqado', user ?? 'AVOQADO')`, payload `{ orderFolio, movementId, reason?, user? }`). Combinado con el fix SQL de 5b (placeholder `bitacorasistema` `'...'` removido + recálculo de cabecera), **H-18 queda RESUELTO**: `cancelOrderItem` es ahora correcto **y** alcanzable. **(2)** `Order.SPLIT` ahora está **cableado** vía un nuevo método público `splitOrder(parentFolio, splitRatio)` (añadido al adapter y a `IPOSAdapter`) que orquesta en **una sola transacción** los helpers antes muertos `createSplitOrder` + `splitOrderItems`, valida `0 < splitRatio < 1`, deriva el monto hijo del total padre y recalcula **ambas** cabeceras (padre e hijo) desde sus líneas vía un nuevo helper privado `recalcOrderTotals(folio, tx)` (mismo patrón SUM-sobre-`tempcheqdet` de `addItemToOrder`/`cancelOrderItem`); `commander.ts` tiene case `Order.SPLIT` → `adapter.splitOrder(orderFolio, splitRatio)`, payload `{ orderFolio, splitRatio }`. Esto **aborda M-20** para la ruta cableada (recalc de cabecera padre+hijo). **Validado en vivo solo en sr11 (v11):** orden de 2 líneas (total 232) dividida a ratio 0.5 → hijo de 2 líneas total 116, padre reducido a 116; luego cancel del movimiento 1 del padre → padre 1 línea total 58 + fila `tempcancela`. **Sigue abierto / NO sobre-afirmar:** split y cancel **sin idempotencia de operación** (re-entrega de `Order.SPLIT` crea otro hijo; re-entrega de `OrderItem.CANCEL` sobre línea ya removida hace que `cancelOrderItem` lance "no encontrado" → DLQ, no no-op limpio; mitigado por el dedup de comando de 5a-2 solo si el backend envía `commandId` estable); rama v10 del split **no validada en vivo** (estructuralmente v10/v11-safe porque `createSplitOrder`/`splitOrderItems` nunca referencian `WorkspaceId`). **H-26 solo PARCIAL:** split ✅; **merge/transfer/cambiar mesa siguen sin implementar**. Cancel de **orden completa**/void (H-22/H-23), modificadores/descuento-ítem/comentario/curso (H-24/H-25) y la implementación real de `closeShift` **siguen pendientes**. Los helpers `createSplitOrder`/`splitOrderItems` **ya tienen caller** (`splitOrder`) — dejan de ser código muerto.
...
5. ~~**`cancelOrderItem`**: contiene SQL placeholder `'...'`, no recalcula totales y no está cableado.~~ **✅ HECHO (Fase 5b + 5c):** SQL placeholder removido + recálculo de cabecera (5b), `OrderItem.CANCEL` cableado (5c). Residual: sin idempotencia de operación (re-entrega sobre línea ya removida → DLQ, no no-op limpio).
...
| Cancelar producto/línea | `OrderItem.CANCEL` ✅5c | `cancelOrderItem` (`:129`, **cableado en 5c**) | ✅ works (caveat idempotencia) | DELETE `tempcheqdet`; INSERT `tempcancela`; recalcula totales de cabecera (✅5b); `bitacorasistema` `'...'` removido (✅5b) | tx OK; ✅ DLQ (5a-1); ❌ idempotencia de operación (re-entrega sobre línea ya removida → lanza → DLQ); folio bind `sql.BigInt` ✅5d | agnóstico; cableado ✅5c |
| Cancelar orden completa | *(ninguno)* | *(no existe)* | ⛔ not-implemented | — | `default` → throw → DLQ (✅5a-1) | — |
| Modificadores / descuento ítem / comentario-edit / curso | *(ninguno)* | *(no existe; solo comentario en alta)* | ⛔ not-implemented | — | `default` → throw → DLQ (✅5a-1) | — |
| Propina (ajuste) | *(ninguno)* | solo columna en pago; `createFastPayment` hardcodea `propina=0` | ⛔ not-implemented | — | — | — |
| Dividir cuenta | `Order.SPLIT` ✅5c | `splitOrder` (`5c`, orquesta `createSplitOrder`+`splitOrderItems` en 1 tx) | ✅ works (caveat idempotencia, v11 only) | valida `0<ratio<1`; deriva monto hijo del total padre; mueve líneas a cheque hijo; recalc cabecera padre **e** hijo vía `recalcOrderTotals` (✅5c, aborda M-20) | 1 tx con rollback OK; ✅ DLQ (5a-1); ❌ idempotencia (re-entrega crea otro hijo) | estructuralmente v10/v11-safe (no toca `WorkspaceId`); **validado solo v11** |
| Juntar / transferir / cambiar mesa | *(ninguno)* | *(no existe)* | ⛔ not-implemented | — | `default` → throw → DLQ (✅5a-1) | — |
| Imprimir cuenta (pre-cuenta) | *(ninguno)* | *(no existe; solo efecto colateral de pago/fast)* | ⛔ not-implemented | — | `default` → throw → DLQ (✅5a-1) | — |
...
**✅ ARREGLADO (Fase 5b + 5c):** SQL corregido en 5b (placeholder `bitacorasistema` `'...'` removido + recálculo de cabecera) y comando cableado en 5c. `commander.ts` tiene case `OrderItem.CANCEL` (con llaves) → `adapter.cancelOrderItem(orderFolio, movementId, reason ?? 'Cancelado vía Avoqado', user ?? 'AVOQADO')`, payload `{ orderFolio, movementId, reason?, user? }`. Combinado, `cancelOrderItem` es ahora correcto **y** alcanzable. **Residual:** sin idempotencia de operación — re-entrega sobre una línea ya removida hace que `cancelOrderItem` lance "no encontrado" → va a la DLQ de comandos (no pérdida silenciosa, pero tampoco no-op limpio); mitigado por el dedup de comando 5a-2 si el backend envía un `commandId` estable.
- **Categoría:** missing-feature · **Evidencia (original):** `commander.ts:53-131` solo tenía `OrderItem.CREATE`; sin case de cancel. `cancelOrderItem` definido en `SoftRestaurant11Adapter.ts:129`, declarado en `IPosAdapter.ts:98`. Un comando de cancel caía en `default` → warn → **ack** como si se hubiera procesado.
- **Recomendación:** ✅ Resuelto. Pendiente: idempotencia de operación ("línea ausente" = no-op exitoso, no DLQ).
...
#### H-20 — `cancelOrderItem`: recálculo de totales de cabecera tras borrar la línea
**✅ ARREGLADO (Fase 5b):** tras el DELETE, `cancelOrderItem` ahora ejecuta el mismo UPDATE por SUM que `addItemToOrder` (manejando el caso orden vacía → totales 0) dentro de la misma transacción, y se quitó el comentario engañoso que afirmaba que lo hacía el trigger. Validado en vivo (5c): tras cancelar 1 de 2 líneas, la cabecera quedó en el total de la línea restante.
- **Categoría:** completeness · **Evidencia (original):** `SoftRestaurant11Adapter.ts:151-175` (DELETE luego commit, sin UPDATE de `tempcheques`; comentario L175 afirmaba que el trigger recalculaba). `Trg_Avoqado_OrderItems` (`01-COMPLETE-INSTALL.sql:711-765`) solo escribe `AvoqadoTracking`, no recalcula.
- **Recomendación:** ✅ Resuelto.
...
#### H-23 — Cancelar orden completa: el comando desconocido se dead-letterea (ya no se ackea en silencio)
**✅ RESUELTO (Fase 5a-1)** para el faceta de manejo de fallos: el `default` ahora **lanza** → un `Order.CANCEL` (sin handler) va a la DLQ de comandos en vez de ackearse en silencio. **Pendiente:** la operación en sí (cancelar orden completa) sigue sin implementar (H-22).
- **Categoría:** failure-handling · **Evidencia (original):** `commander.ts:129-130` (default: solo `log.warn`, sin throw) → `:134` `channel.ack(msg)`.
- **Recomendación:** ✅ Resuelto (5a-1). Implementar el handler real es H-22.
...
#### H-25 — Modificadores/descuentos: comandos desconocidos se dead-letterean (ya no se ackean en silencio)
**✅ RESUELTO (Fase 5a-1):** el `default` ahora lanza → estos comandos sin handler van a la DLQ. Implementar las operaciones es H-24.
- **Categoría:** failure-handling · **Evidencia (original):** `commander.ts:129-130` default warn → ack `:134`. · **Recomendación:** ✅ Resuelto (5a-1); el handler real es H-24.

#### H-26 — Dividir/juntar/transferir/cambiar mesa: split ✅ (Fase 5c, validado v11; v10 no validado); juntar/transferir/cambiar mesa PENDIENTES
**◑ PARCIAL (Fase 5c):** **split cableado** vía `splitOrder(parentFolio, splitRatio)` (nuevo método público en el adapter y en `IPOSAdapter`) que orquesta en **una sola tx** los helpers antes muertos `createSplitOrder` + `splitOrderItems`, valida `0 < splitRatio < 1`, deriva el monto hijo del total padre y recalcula **ambas** cabeceras (padre e hijo) vía `recalcOrderTotals(folio, tx)` (✅ aborda M-20 para la ruta cableada). `commander.ts` tiene case `Order.SPLIT` → `adapter.splitOrder(orderFolio, splitRatio)`, payload `{ orderFolio, splitRatio }`. Validado en vivo solo en **sr11 (v11)**. **Pendientes: juntar (merge), transferir ítems, cambiar mesa.**
- **Categoría:** missing-feature · **Evidencia (original):** `commander.ts:53-131` (6 comandos, sin split/merge/transfer/changeTable); `IPosAdapter.ts:90-109` sin firma. Los helpers `createSplitOrder`/`splitOrderItems`/`adjustOrderItemQuantities`/`updateParentOrderTotal` tenían **cero callers**.
- **Estado actual:** `createSplitOrder`/`splitOrderItems` **ya tienen caller** (`splitOrder`) — dejan de ser código muerto. `adjustOrderItemQuantities`/`updateParentOrderTotal` **siguen sin uso** (`splitOrder` recalcula vía `recalcOrderTotals`, NO vía esos helpers).
- **Caveats:** sin idempotencia de operación (re-entrega de `Order.SPLIT` crea otro hijo); rama **v10 no validada en vivo** (estructuralmente v10/v11-safe porque `createSplitOrder`/`splitOrderItems` nunca referencian la columna `WorkspaceId`, que en v11 tiene `DEFAULT(newid())` y en v10 no existe).
- **Recomendación:** Para merge/transfer/changeTable decidir alcance y diseñar comandos explícitos (`Order.MERGE`, `OrderItem.TRANSFER`, `Order.CHANGETABLE`) + métodos transaccionales que repliquen el SQL nativo (contadores `folios`, ocupación `mesas/mesasasignadas`, re-apuntar `foliodet`, recalc de totales padre/hijo). Validar la rama v10 del split en un DB v10 real. Agregar idempotencia de operación al split.
...
#### H-28 — Imprimir cuenta / cualquier comando futuro: comando desconocido se dead-letterea (ya no se ackea)
**✅ RESUELTO (Fase 5a-1):** el `default` ahora lanza → cualquier comando futuro sin handler va a la DLQ en vez de tragarse en silencio. Implementar imprimir cuenta es H-27.
- **Categoría:** failure-handling · **Evidencia (original):** `commander.ts:129-130` default warn → ack `:134`. · **Recomendación:** ✅ Resuelto (5a-1); el handler real es H-27.
...
| M-20 | table-ops | **◑ ABORDADO en la ruta cableada (Fase 5c):** el riesgo de corrupción de totales se abordó vía el orquestador `splitOrder`, que recalcula **ambas** cabeceras (padre e hijo) desde sus líneas vía `recalcOrderTotals` (NO usa el helper defectuoso `updateOrderTotal` con `taxRate 0.16`). **Residual:** sin idempotencia de operación; validado solo v11; los helpers `updateOrderTotal`/`adjustOrderItemQuantities`/`updateParentOrderTotal` siguen muertos. | `SoftRestaurant11Adapter.ts:670-744,605-633` (helpers via `splitOrder` ✅5c); `updateOrderTotal:793` (taxRate 0.16, sin uso) | Residual: agregar idempotencia al split; validar v10; eliminar los helpers de totales aún muertos |
| M-21 | table-ops / print | **✅ RESUELTO (Fase 5a-1):** comandos desconocidos (merge/print futuros) ahora se dead-letterean (default lanza → DLQ), ya no ackeados-y-dropeados. (split ya es handler propio desde 5c.) | `commander.ts` default lanza ✅5a-1; DLQ de comandos ✅5a-1 | ✅ Resuelto |
...
1. ~~**Cancelar línea de ítem**~~ — **✅ HECHO (Fase 5b + 5c):** `cancelOrderItem` corregido (5b: SQL placeholder removido + recalc) y cableado a `OrderItem.CANCEL` (5c). Residual: sin idempotencia de operación. (P0 — resuelto)
...
12. ~~**Dividir cuenta**~~ — **✅ HECHO (Fase 5c):** `Order.SPLIT` → `splitOrder` orquesta `createSplitOrder/splitOrderItems` (ya no muertos) + recalc padre/hijo. Validado v11; v10 no validado; sin idempotencia de operación. (P2 — resuelto)
...
- **Comando desconocido = ack-and-drop (HIGH).** **✅ RESUELTO (Fase 5a-1):** el `default` ahora **lanza** en vez de ackear → el comando desconocido va a la DLQ de comandos (validado en vivo). Antes (`:129-130`) solo hacía `log.warn` y caía a `channel.ack(msg)`, tragándose en silencio todo comando no implementado o con typo. (Nota: `OrderItem.CANCEL` y `Order.SPLIT` ya **no** caen en `default` — fueron cableados en Fase 5c.)
...
> **Estado:** **Fase 5a — HECHA** (detener el sangrado a nivel transporte): DLQ de comandos (5a-1, ✅) + idempotencia a nivel Commander vía `AvoqadoProcessedCommands` (5a-2, ◑ parcial — requiere `commandId` estable del backend, hoy fail-open). **Fase 5b — HECHA (parcial):** remoción del SQL placeholder `'...'` de `cancelOrderItem`/`bitacorasistema` + recálculo de totales (la implementación real de `closeShift` sigue pendiente; el cableado de `cancelOrderItem` se completó en 5c). **Fase 5c — HECHA (parcial):** cableado de `OrderItem.CANCEL`→`cancelOrderItem` y `Order.SPLIT`→`splitOrder` (nuevo orquestador que envuelve los helpers existentes `createSplitOrder`/`splitOrderItems` + `recalcOrderTotals`), validado v11; pendiente el resto de 5c (cancelar orden completa/void, modificadores, descuentos, imprimir, merge/transfer/cambiar mesa) y la implementación real de `closeShift`. **Fase 5d — HECHA** (quitar hardcodes de dinero/versión vía `posMeta.ts`): `Order.CREATE`/`openShift` version-aware (H-9 ✅, rama v10 pendiente de validación en vivo), IVA de `addItemToOrder` desde `parametros.impuesto1`, `posAreaId`/`idempresa` de config en `Order.CREATE`, `applyPayment` INSERT real (cierra H-19 con 5b), folios `sql.Int`→`sql.BigInt`. **Pendiente: Fase 5b** (resto — `closeShift` placeholder/destructivo) y **resto de Fase 5c** (cancel orden completa/void, modificadores/descuentos, imprimir, merge/transfer/cambiar mesa).
...
6. **`cancelOrderItem`:** ✅ HECHO — placeholder `bitacorasistema` `'...'` removido + recálculo de totales tras el DELETE (Fase 5b); `OrderItem.CANCEL` cableado (Fase 5c). **Pendiente:** idempotencia ("línea ausente" = no-op exitoso, no DLQ), auditoría de cancelación a observe (M-13), match KDS (L-11). (H-18 ✅, H-19 ✅, H-20 ✅; M-13, M-14, L-11 pendientes)
...
10. **Dividir/juntar/transferir/cambiar mesa** — **split ✅ HECHO (Fase 5c):** `Order.SPLIT`→`splitOrder` orquesta `createSplitOrder`/`splitOrderItems` + recalc padre/hijo (`recalcOrderTotals`), validado v11. **Pendientes:** merge/transfer/changeTable (comandos explícitos con SQL nativo fiel); validar la rama v10 del split; agregar idempotencia de operación al split. (H-26 parcial, M-20 abordado)
...
23. **Limpiar código muerto** — `createSplitOrder`/`splitOrderItems` **ya NO son muertos** (los usa `splitOrder` desde Fase 5c). Siguen sin uso: `updateOrderTotal`/`adjustOrderItemQuantities`/`updateParentOrderTotal`/`markOrderAsPaid`/`insertPaymentToPOS`/`getOrderData`/resolvers duplicados, `applyPayment`/`closeAndPayOrder` legacy. (M-18, M-20)
