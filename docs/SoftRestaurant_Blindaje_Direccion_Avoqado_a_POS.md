...
|---|---|---|---|---|---|---|
| Abrir cuenta/mesa | `Order.CREATE` | `createEmptyOrder` → `services/Orders/createEmptyOrder.ts` | ⚠️ caveats | INSERT `tempcheques` (idturno=0, `idarearestaurant='01'`, `idempresa='1'`, WorkspaceId); +1 `folios.ultimaorden`; UPDATE totales=0 **post-commit fuera de tx** | tx con rollback **parcial** (guard de ocupación y totales fuera de la tx); ❌ DLQ; ❌ idempotencia | v11/v12 OK; **v10 ROTO** (columna WorkspaceId hardcodeada) |
| Agregar producto | `OrderItem.CREATE` | `addItemToOrder` (`:33`) | ⚠️ caveats | lookup producto por **`descripcion`** (no idproducto); INSERT `tempcheqdet` (IVA 16% hardcode, `idestacion='AVOQADO_SYNC'`); recalc cabecera por SUM | tx con rollback OK; ❌ DLQ; ❌ idempotencia; **recalc cabecera mal si cantidad>1** | mismo SQL en todas; WorkspaceId vía DEFAULT; folio bind `sql.Int` (BIGINT real) |
...
| Abrir turno | `Shift.OPEN` | `openShift` (`:239`) | ⚠️ caveats | INSERT `turnos` (idturno=MAX+1 sin lock, `cajero`=staffId, `idmesero=''`, `idempresa='1'`); UPDATE `parametros.ultimoturno` **sin WHERE** | tx con rollback OK; ❌ DLQ; ❌ idempotencia (sin guard `cierre IS NULL`) | INSERT no v10-safe; staffId publicado vacío en todas |
...
| Cancelar producto/línea | *(ninguno)* | `cancelOrderItem` (`:129`, declarado pero **NO cableado**) | ❌ broken / ⛔ no-wired | DELETE `tempcheqdet`; INSERT `tempcancela` + `bitacorasistema` con **`'...'`**; **no recalcula totales** | tx OK pero INSERT placeholder lanza; ❌ DLQ; ❌ idempotencia | agnóstico; muerto hoy |
...
| `applyPayment` / `closeAndPayOrder` (legacy) | *(ninguno)* | `:188` / `:206` (declarados, **no cableados**, `'...'`) | ⛔ no-wired (placeholder) | INSERT `tempchequespagos` con `'...'` (no ejecutable) | sin tx (applyPayment) | — |
...
- **Categoría:** version-compat · **Evidencia:** `createEmptyOrder.ts:41,45,54` hardcodean `WorkspaceId` en la lista de columnas y VALUES sin rama de versión. En un esquema v10 genuino (sin columna `WorkspaceId`) el INSERT falla con "Invalid column name WorkspaceId". Contradice el requisito duro de CLAUDE.md (todo cambio debe funcionar en v10 Y v11).
- **Recomendación:** Detectar versión (`fn_GetSoftRestaurantVersion` / `AvoqadoConfig.HasWorkspaceId`) y omitir la columna en v10, o construir la lista dinámicamente. Verificar contra el DB de prueba v10.
...
#### H-19 — `cancelOrderItem`: el INSERT a `bitacorasistema` tiene placeholders `'...'` → T-SQL inválido que lanza y hace rollback del void completo
- **Categoría:** correctness · **Evidencia:** `SoftRestaurant11Adapter.ts:167-171` `.query('INSERT INTO bitacorasistema (fecha, usuario, evento, valores, ...) VALUES(GETDATE(), @usuario, @evento, @valores, ...)')`. El mismo anti-patrón `'...'` existe en `applyPayment` (`:199`).
- **Recomendación:** Reemplazar por la lista real de columnas + parámetros (capturar el set nativo con `npm run monitor`), o quitar la escritura a `bitacorasistema`. No enviar SQL placeholder en una ruta de ejecución.
...
- **Categoría:** correctness · **Evidencia:** `SoftRestaurant11Adapter.ts:1051-1053` (`totalimpuesto1=0`, `subtotalsinimpuestos=@amount`), `:1077-1090` (`precio=preciosinimpuestos=@amount`, `impuesto1/2/3=0`). Contradice `addItemToOrder` que divide 16% (`:59`).
- **Por qué importa:** El IVA de cada venta rápida se reporta como 0 y la base neta se sobre-estima → totales de impuesto corrompidos y cruces fiscales/CFDI rotos. Internamente inconsistente con las órdenes normales.
- **Recomendación:** Computar el impuesto con la tasa configurada del venue (no hardcodear 16%): `preciosinimpuestos = amount/(1+rate)`, `impuesto1 = amount - preciosinimpuestos`. Reusar la misma derivación que `addItemToOrder`.
...
- **Categoría:** hardcode · **Evidencia:** `SoftRestaurant11Adapter.ts:1027` (producto default `'FASTPAY'`), `:1047-1049` (`mesa='FAST'`, `idarearestaurant='01'`, `idempresa='0000000001'`), `:1076` (INSERT `tempcheqdet` con idproducto literal). `createEmptyOrder.ts:45` usa `idempresa='1'` para la misma operación (**inconsistencia entre las dos rutas**).
...
|---|---|---|---|---|
| M-1 | Shift.OPEN | `idturno = MAX(idturno)+1` sin lock → carrera con altas nativas/comandos concurrentes | `SoftRestaurant11Adapter.ts:247` (sin hint de lock) | Leer MAX bajo `UPDLOCK,HOLDLOCK` (o serializable), o derivar el número como el POS nativo (contador bajo `TABLOCKX`) |
...
| M-10 | Order.CREATE | Hardcodes `idarearestaurant='01'`, `idempresa='1'`, estación, y `posAreaId` ignorado | `createEmptyOrder.ts:44-45` (literales); `data.posAreaId` nunca leído (existe en `IPosAdapter.ts:35`) | Usar `data.posAreaId`; resolver `idempresa` de config; validar existencia de área/empresa |
...
| M-12 | Payment.APPLY | IVA 16% hardcodeado como fallback en recálculo parcial (latente, vía helper TS) | `SoftRestaurant11Adapter.ts:810` (`taxRate = 0.16`), `:59` | Quitar el fallback 0.16; derivar la tasa de los campos de impuesto/config; fallar fuerte si no se puede derivar |
...
| M-18 | tx-integrity | Métodos legacy inalcanzables con SQL placeholder y sin tx (`applyPayment`, `bitacorasistema` de cancelOrderItem) | `SoftRestaurant11Adapter.ts:198-200` (applyPayment `'...'` sin tx), `:171` | Borrar los legacy inalcanzables o terminarlos con listas de columnas válidas dentro de tx |
...
| L-4 | OrderItem.CREATE | `folio` bindeado como `sql.Int` mientras las columnas son BIGINT | `SoftRestaurant11Adapter.ts:56,66,109` | Bindear `folio/foliodet` como `sql.BigInt` en toda la ruta de comandos |
...
- **Código muerto con SQL placeholder (MEDIUM, M-18):** `applyPayment` (`:198-200`, sin tx, `'...'`), `cancelOrderItem` `bitacorasistema` (`:171`). Bombas latentes el día que se cableen.
...
- **Hardcodes que rompen multi-tenant:** IVA 16% (`addItemToOrder:59,63`; `createFastPayment` IVA=0; helper `updateOrderTotal:810`), `idarearestaurant='01'`, `idempresa` inconsistente (`'1'` en `createEmptyOrder` vs `'0000000001'` en `createFastPayment`), `serie=''`, estación `'AVOQADO_SYNC'`, `MAX+1` sin lock para `idturno`/`movimiento`/`numcheque`, `folio` como `sql.Int` sobre BIGINT.

---

## 6. Plan de blindaje priorizado (Fase 5.x)

> **Estado:** **Fase 5a — HECHA** (detener el sangrado a nivel transporte): DLQ de comandos (5a-1, ✅) + idempotencia a nivel Commander vía `AvoqadoProcessedCommands` (5a-2, ◑ parcial — requiere `commandId` estable del backend, hoy fail-open). **Pendiente: Fase 5b** (arreglar lo roto — `closeShift` placeholder/destructivo y `cancelOrderItem` roto/no cableado) y **Fase 5c** (implementar operaciones faltantes — cancel orden, modificadores/descuentos, imprimir, split/merge/transfer/cambiar mesa).
...
16. **`Order.CREATE` version-aware** (omitir `WorkspaceId` en v10) + usar `posAreaId` + `idempresa` de config + ocupación nativa + totales dentro de la tx. (H-9, M-8, M-9, M-10, M-17)
17. **Reconciliar `idempresa`** entre `createEmptyOrder` (`'1'`) y `createFastPayment` (`'0000000001'`); derivar de config. (H-31)
18. **Quitar fallback IVA 0.16** del helper TS; derivar tasa de impuesto/config. (M-12)
19. **`idturno`/`movimiento`/`numcheque` bajo lock**; bindear folios como `sql.BigInt`; parametrizar `idturno` interpolado. (M-1, L-3, L-4, L-8)
